...
export class FoodsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.api.getMenuItems(this.barId,"Speisekarte").pipe(map((actions : any) => {
      return actions.map(a => {
        const data = a.payload.doc.data()
        const id = a.payload.doc.id;
        return { id, ...data };
  ...
  editCategory() {
    if(this.selectedCategory != ""){
      this.router.navigate(['/bar/karten/speisen/kategorie-anpassen']);
    }
  }
  editMenu(Item) {
    this.auth.saveMenuItem(Item);
    this.router.navigate(['/bar/karten/speisen/eintrag-anpassen']);
  }

  navigateNewCategory() {
    this.router.navigate(['/bar/karten/speisen/kategorie-erstellen']);
  }

  navigateNewMenuItem() {
    this.router.navigate(['/bar/karten/speisen/eintrag-erstellen']);
  }
